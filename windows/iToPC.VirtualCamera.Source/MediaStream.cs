...
public partial class MediaStream : IMFAttributes, IMFMediaStream2, IMFMediaStream, IMFMediaEventGenerator, IKsControl, IDisposable
{
    ...
    public MediaStream(MediaSource source, uint index)
    {
        try
        {
            ...
            var mediaTypes = new IMFMediaType[1];
            ...
            nv12Type.Set(Constants.MF_MT_DEFAULT_STRIDE, (uint)NUM_IMAGE_COLS);
    ...
    {
        // Shared frames live in CPU memory. Do not attach the supplied D3D
        // manager so the allocator returns a lockable system-memory sample.
        return Constants.S_OK;
    }
