...
iPhone→PC間と仮想カメラ出力は最大3840×2160/120fpsです。仮想カメラは4K/120fps NV12として公開されます。入力が4K/120未満の場合も出力形式は4K/120のままですが、FFmpegによる拡大とフレーム複製になるため、実際のディテールと時間解像度はiPhone側で選ばれた形式が上限です。
...
仮想カメラの公開形式は4K/120fpsですが、現行のCPU共有メモリ経路ではPCのメモリ帯域や利用先アプリによって実効fpsが下がります。iPhone側はエンコーダー待ちを最大2フレームに制限し、処理落ち時は古いフレームを捨ててメモリ増加と遅延の累積を防ぎます。
