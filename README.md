...
iPhone→PC間は最大4K/120で受信しますが、一般的な会議・配信アプリとの互換性とメモリ帯域を優先し、仮想カメラ出力は1920×1080/60fps NV12です。仮想カメラをオフにすると、従来どおりffplayの別ウィンドウで最大解像度をプレビューできます。

映像が出ずffmpegまたはffplayが終了する場合は、Windows側の `D3D11VA ハードウェアデコード` をオフにして確認します。ソフトウェアデコードでは4K/120の維持は困難です。
